...
public class FlightCommands {
    ...
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        ...
        dispatcher.register(Commands.literal("flight")
            ...
            .then(Commands.literal("time")
                .executes(FlightCommands::toggleFlightTimeDisplay))
    ...
    private static int toggleFlightTimeDisplay(CommandContext<CommandSourceStack> context) {
        CommandSourceStack source = context.getSource();
        ...
        Boolean enabled = FlightManager.toggleFlightTimeHud(player);
        if (enabled == null) {
            source.sendFailure(Component.literal("§c✦ 你的客户端未安装显示组件，无法开启飞行时间显示。 §c✦"));
            return 0;
        }

        if (enabled) {
            source.sendSuccess(() -> Component.literal("§a✦ 已开启飞行时间显示！ §a✦"), false);
        } else {
            source.sendSuccess(() -> Component.literal("§c✦ 已关闭飞行时间显示！ §c✦"), false);
        }
