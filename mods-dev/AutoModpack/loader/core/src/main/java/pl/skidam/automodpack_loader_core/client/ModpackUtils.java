...
public class ModpackUtils {
    ...
    public static Function<X509Certificate, Boolean> userValidationCallback(InetSocketAddress address, boolean allowAskingUser) {
        // AUTO-TRUST: Skip fingerprint verification and automatically trust all server certificates
        return certificate -> {
            String fingerprint;
            try {
                fingerprint = NetUtils.getFingerprint(certificate);
            } catch (CertificateEncodingException e) {
                LOGGER.warn("Could not get fingerprint from certificate, but auto-trusting anyway");
                return true;
            }
            // Auto-save the fingerprint to known hosts for future reference
            knownHosts.hosts.put(address.getHostString(), fingerprint);
            ConfigTools.save(knownHostsFile, knownHosts);
            LOGGER.info("Auto-trusted certificate from server {}, fingerprint: {}", address.getHostString(), fingerprint);
            return true;
        };
    }
